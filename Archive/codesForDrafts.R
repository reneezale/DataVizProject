...
#vaxplot3 = vaxplot2 + scale_y_continuous(breaks=c(0,10, 20, 30, 40),
                                   #limits = c(0, 40), 
                                   #labels=unit_format(suffix = '%')) #####Not working
#vaxplot3
...
# save vaxplot4 ----------------------------------------------------------
saveRDS(vaxplot4, file = "vaxplot4.rds")


# deliverable 2 ----------------------------------------------------------
#Boxplot or Histogram of age of vaccinated respondents 

head(mydata$NewVaxstatus)

library(dplyr)

#Filtering dataset to only vaccinated respondents
VaxedData <- filter(mydata, NewVaxstatus == "Received")

#basic boxplot
baseAge= ggplot(VaxedData,aes(y = age))  #var to plot in 'y' 
BoxplotAge <- baseAge + geom_boxplot()
BoxplotAge

#Getting boxplot labels
(statVals=summary(VaxedData$age,digits = 3)[1:6])

#adding boxplot labels
statVals=statVals%>%as.vector() 
baseVaxedAge= ggplot(VaxedData,aes(y = age))  
b1= baseVaxedAge + geom_boxplot() 
...
#Re-doing Y Axis label and adding other labels
y.AxisTextAge="Age"
...
VaxAgeBoxPlot = b1_annot_noX + labs(title=VaxtitleText,
                           y = y.AxisTextAge,
                           caption = sourceText) 

VaxAgeBoxPlot

#Tilting Y Axis labels so they fit

VaxAgeBoxPlot <- VaxAgeBoxPlot + theme(axis.text.y = element_text(angle = 45, hjust = 1))
VaxAgeBoxPlot


#Look into making boxplot of age of unvaccinated people
######################Age Histogram#########################
barWIDTH=15
library(ggplot2)
...
saveRDS(VaxAgeBoxPlot, file = "VaxAgeBoxPlot.rds")
...
library(ggplot2)
ggplot(data=VaxedByPoliticsDF, 
       aes(x=PoliticalAffiliation, y=counts)) + geom_bar(stat="identity")

base1=ggplot(data=VaxedByPoliticsDF,
             aes(x=PoliticalAffiliation, y=counts,
                 fill=VaxStatus)) # fill brings a legend
base1
barDodge1= base1 +  geom_bar(stat="identity",
                             position ='dodge') 
barDodge1 

#####Getting NA for Received even though relabeled in Excel, all should have labels#####
#VaxedByPoliticsDF$VaxStatus2 <- factor(VaxedByPoliticsDF$VaxStatus, levels=c('Received', 'Planning to', 'No plans', 'Im not sure'))
#VaxedByPoliticsDF$VaxStatus2

#Reordering X axis

#PolAxisOrder <- c(3, 1, 2, 4)
#VaxedByPoliticsDF$PolAxisOrder=as.vector(PolAxisOrder)
#VaxedByPoliticsDF

    
#making new base
base2=ggplot(data=VaxedByPoliticsDF,
             aes(x= PoliticalAffiliation,
                 y=pctCol, # % not counts
                 fill=VaxStatus)) + theme_minimal()
barDodge2= base2 +  geom_bar(stat="identity",
                             position ='dodge') 

barDodge2 + labs(title = "Politics and Vaccination Plans",
                 subtitle = "COVID-19 Vaccination Status by Political Affiliation",
                 x="",
                 y="Share of population",
                 fill="New COVID-19 Vaccination Plans",
...
barStacked2
#####Attempting reorder of variables#####
#####Doesnt seem to like 'reorder'#####

VaxedByPoliticsDF
base3=ggplot(data=VaxedByPoliticsDF, 
             aes(x=PoliticalAffiliation, y=pctCol,
                 fill=reorder(PoliticalAffiliation,pctCol))) 
...
# save del3Draft ----------------------------------------------------------
saveRDS(barStacked3, file = "StackedBarFail.rds")
saveRDS(barStacked2, file = "StackedBarNoOrder.rds")
